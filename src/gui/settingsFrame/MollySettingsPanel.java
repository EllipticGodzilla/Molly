...
//pannello per SettingsFrame mostrato quando si preme file>settings nella menu bar in MFrame
public class MollySettingsPanel implements SettingsPanel {
    ...
    public static void init() {
        ...
        GraphicsSettings.run_at_theme_change(MollySettingsPanel::update_colors_static);
        ...
        ThemesSettings.init();
        add_item("graphics/theme settings", new ThemesSettings(), ThemesSettings.ok_listener, ThemesSettings.apply_listener);
    }
    ...
    public static boolean add_item(String path, SettingsPanel panel, ActionListener ok_listener, ActionListener apply_listener) {
        ...
        if (root_menu_len == -1) {
            Logger.log("impossibile aggiungere CascateItem al Settings panel, specificare un root menu per: " + path, true);
            return false;
        }
        else {
...
class ThemesSettings implements SettingsPanel {
    ...
    public static void init() {
        ...
        update_colors_static();
        GraphicsSettings.run_at_theme_change(ThemesSettings::update_colors_static);
